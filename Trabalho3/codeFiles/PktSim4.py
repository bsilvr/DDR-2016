...
for i in pkts_recv:
    for j in queue_size:
        ...
        node2=Node(env,'N2',300, k)
        link1=Link(env,'L1',10e9,k)
        link2=Link(env,'L2',10e9,k)
        ...
        #    Atraso na fila               + atraso router1  + atraso router2
        W = (lambd)/(2.0*miu*(miu-lambd)) + (1/miu)         + (1/miu)
        # Router 2 apenas tem de se considerar o atraso de processamento uma vez que os pacotes já saem do router1 com uma taxa constante e igual ao router 2 e como lambda router2 = miu router1 => miu-lambda = 0 a formula não ia ser possivel dividir por zero.

        print "Theorical average delay: %f"%W
        print "Theorical packet loss: %.2f%%" % (0)

        link1_loss = (100.0*node1.lost_pkts/tx.packets_sent)
        link2_loss = (100.0*node2.lost_pkts/tx.packets_sent)
